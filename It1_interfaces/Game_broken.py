...
from It1_interfaces.Command import C                    if stationary_pieces and moving_pieces:
                        ...
                        for moving_piece in moving_pieces:
                            ...
                            moving_piece.on_command(idle_cmd, now)rom It1_interfaces.Piece import Piece
            ...
            if len(pieces_in_cell) > 1:
                ...
                if len(white_pieces) > 1:
                    ...
                    if stationary_pieces and moving_pieces:
                        # Block the moving piece by resetting it to its original position
                        for moving_piece in moving_pieces:
                            print(f"[DEBUG] � Blocking movement of {moving_piece.piece_id} - friendly fire prevention")
                            # Reset the moving piece to its starting position
                            if hasattr(moving_piece.current_state.physics, 'start_cell'):
                                moving_piece.current_state.physics.current_cell = moving_piece.current_state.physics.start_cell
                                moving_piece.current_state.physics.target_cell = moving_piece.current_state.physics.start_cell
                                moving_piece.current_state.physics.is_moving = False
                    elif len(white_pieces) > 1:
                        # If we can't determine which is moving, block all but the first by resetting their positions
                        print(f"[DEBUG] ⚠️  Fallback: Blocking all white pieces except first at {pos}")
                        for p in white_pieces[1:]:
                            if hasattr(p.current_state.physics, 'start_cell'):
                                p.current_state.physics.current_cell = p.current_state.physics.start_cell
                                p.current_state.physics.target_cell = p.current_state.physics.start_cell
