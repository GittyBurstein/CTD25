...
"""🎯 tests comprehensive לfunctionality of get_state_after_command"""
...
"""🧪 if no ניתן לעבור transition, מחזיר the עצמו"""
# הגדרת rest state שעדיין no סיים
...
"""🧪 transition valid יוצר state new"""
# הגדרת target state
...
"""🧪 number transitions - בחירת הנכון by type"""
# הוספת number transitions
...
# exactly בזמן שיכול לעבור transition (500 + 1000 = 1500)
...
"""🔄 tests comprehensive לfunctionality of update"""
...
"""🧪 update קורא for each הcomponents"""
        now_ms = 1000
...
# ללא transitions - צריך להחזיר the עצמו
        self.assertEqual(result, self.state)
        
    @patch('It1_interfaces.State.GraphicsFactory')
    def test_update_movement_complete_transition(self, mock_graphics_factory):
"""🧪 update with השלמת movement -> transition"""
        now_ms = 1000
...
# צריך לעבור לstate new because timeout
        self.assertNotEqual(result, self.state)
        self.assertEqual(result.state, "idle")
        
    def test_update_rest_state_no_timeout_yet(self):
"""🧪 update of rest state שעדיין no timeout"""
        now_ms = 1500
...
# no צריך לעבור transition עדיין
        self.assertEqual(result, self.state)
        
    @patch('It1_interfaces.State.GraphicsFactory')
    def test_update_move_state_auto_completion(self, mock_graphics_factory):
"""🧪 update of move state with auto-completion"""
...
"""🧪 update of move state שעדיין זז"""
        self.state.state = "move"
...
# no צריך להשלים עדיין
        self.assertEqual(result, self.state)
        
    def test_update_long_rest_state_handling(self):
"""🧪 update of long_rest state"""
...
"""🧪 update with number תנאים - checking סדר עדיפויות"""
        now_ms = 3000
        
# הגדרת state שיכול לטריגר number transitions
        self.mock_physics.update.return_value = True  # Movement complete
        self.state.is_rest_state = True
        self.state.rest_duration_ms = 1000
        self.state.state_start_time = 1000  # Timeout ready
        
# הוספת second transitions
...
"""🏭 tests comprehensive לפונקציות factory of states"""
...
"""🧪 creating long rest state with מאפיינים נכונים"""
...
"""🧪 creating short rest state with מאפיינים נכונים"""
...
"""🧪 creating move state with transition לlong rest"""
...
# checking שlong rest נוצר with הparameters הנכונים
        mock_create_long_rest.assert_called_once_with(
            self.idle_state, self.mock_moves, self.mock_graphics, self.mock_physics
        )
        
# checking complete transition
...
"""🧪 checking שmove state no נוצר כrest state"""
        move_state = create_move_state(
            self.idle_state, self.mock_moves, self.mock_graphics, self.mock_physics
        )
        
# move state no צריך להיות rest state
...
"""🧪 פונקציות factory with idle states different"""
# creating idle states different
...
# creating rest states with idle states different
...
# checking שהtimeout transitions מצביעים לndle states הנכונים
...
"""📋 tests comprehensive לget_command functionality"""
...
"""🧪 get_command אחרי reset with command"""
...
"""🧪 get_command with different command types"""
        commands = [
...
"""🧪 get_command אחרי number resets"""
